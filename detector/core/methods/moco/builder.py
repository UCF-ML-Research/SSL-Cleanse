...
class MoCoLosses(typing.NamedTuple):
    ...
    def combine(self, contr_w: float=1, align_w: float=1, unif_w: float=1) -> torch.Tensor:
        ...
        l = 0
        if contr_w != 0:
            assert self.loss_contr_labeled is not None and self.loss_contr_unlabeled is not None
            l += contr_w * (self.loss_contr_labeled + self.loss_contr_unlabeled)
        if align_w != 0:
...
class MoCo(nn.Module):
    ...
    def forward(self, im_q, im_k, t_im_q, t_im_k, target_label):
        r"""
        Input:
        ...
        q, t_q = self.encoder_q(im_q), self.encoder_q(t_im_q)  # queries: NxC
        q, t_q = F.normalize(q, dim=1), F.normalize(t_q, dim=1)
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            t_im_k, t_idx_unshuffle = self._batch_shuffle_ddp(t_im_k)

            k, t_k = self.encoder_k(im_k), self.encoder_k(t_im_k)  # keys: NxC
            k, t_k = F.normalize(k, dim=1), F.normalize(t_k, dim=1)

            # undo shuffle
            k, t_k = self._batch_unshuffle_ddp(k, idx_unshuffle), self._batch_unshuffle_ddp(t_k, t_idx_unshuffle)
        ...
        def t_get_q_bdot_k():
            if not hasattr(t_get_q_bdot_k, 'result'):
                t_get_q_bdot_k.result = (t_q * t_k).sum(dim=1)
            assert t_get_q_bdot_k.result._version == 0
            return t_get_q_bdot_k.result

        def tt_get_q_bdot_k():
            if not hasattr(tt_get_q_bdot_k, 'result'):
                tt_get_q_bdot_k.result = (q * t_q).sum(dim=1)
            assert tt_get_q_bdot_k.result._version == 0
            return tt_get_q_bdot_k.result
        ...
        # lazyily computed & cached!
        def t_get_q_dot_queue():
            if not hasattr(t_get_q_dot_queue, 'result'):
                t_get_q_dot_queue.result = t_q @ self.queue.clone().detach()
            assert t_get_q_dot_queue.result._version == 0
            return t_get_q_dot_queue.result
        ...
        if self.contr_tau is not None:
            ...
            t_l_pos = t_get_q_bdot_k().unsqueeze(-1)
            t_l_neg = t_get_q_dot_queue()
            t_logits = torch.cat([t_l_pos, t_l_neg], dim=1)
            t_logits /= self.contr_tau

            tt_l_pos = tt_get_q_bdot_k().unsqueeze(-1)
            tt_l_neg = get_q_dot_queue()
            tt_logits = torch.cat([tt_l_pos, tt_l_neg], dim=1)

            moco_loss_ctor_dict['logits_acc'] = logits
            moco_loss_ctor_dict['loss_acc'] = F.cross_entropy(logits, self.scalar_label.expand(logits.shape[0]))
            moco_loss_ctor_dict['logits_asr'] = t_logits
            moco_loss_ctor_dict['loss_asr'] = F.cross_entropy(t_logits, self.scalar_label.expand(t_logits.shape[0]))
            moco_loss_ctor_dict['logits_acc_t'] = tt_logits
